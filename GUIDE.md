...
```bash
pip3 install --upgrade discord.py yt_dlp
```

## Configure the bot token

Open `bot.py` and replace `YOUR_BOT_TOKEN_HERE` with your actual Discord bot token:

```python
bot.run('YOUR_BOT_TOKEN_HERE')
```

> Keep your token secret. Do not publish it or commit it to git.
